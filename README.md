...
The base `rusqlite` package supports SQLite version 3.45.3 or newer. If you need
...
* `omit_load_extension` compiles bundled builds with `SQLITE_OMIT_LOAD_EXTENSION`. It cannot be combined with `load_extension`, and fails the build when SQLite is linked instead of compiled.
